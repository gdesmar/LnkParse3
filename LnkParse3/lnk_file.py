...
class LnkFile(object):
    ...
    def print_lnk_file(self, print_all=False):
        ...
        def print_extra_block_information(extra_key, extra_value, level=0, first_in_list=False) -> bool:
            if isinstance(extra_value, dict):
                """
                if level == 1:
                    cprint(f"{extra_key}:", level)
                elif level == 2:
                    cprint(extra_key, level)
                else:
                    cprint(f"{nice_id(extra_key)}:", level)
                """
                if extra_key is not None:
                    cprint(f"{extra_key if level < 3 else nice_id(extra_key)}{':' if level != 2 else ''}", level)
                sub_first_in_list = first_in_list
                for key, value in extra_value.items():
                    sub_first_in_list = print_extra_block_information(key, value, level + 1, sub_first_in_list)
            ...
            if isinstance(extra_value, list):
                ...
                for item_index, extra_value_item in enumerate(extra_value):
                    ...
                    first_in_list = print_extra_block_information(None, extra_value_item, level, first_in_list)
                return first_in_list
